---
name: elixir-phoenix-authorization-patterns
description: Handles all authorization and access control work in Phoenix and LiveView
  applications. Covers server-side authorization, owner-only patterns, scoped queries,
  policy modules, controller authorization, and authorization testing. Use when writing
  permission checks, policy modules, or role-based access controls, or when addressing
  authorization, access control, permission, policy, role, owner, scoped query, or
  IDOR concerns.
license: MIT
metadata:
  source-id: igmarin/elixir-phoenix-skills:phoenix-authorization-patterns
