---
name: ruby-tdd-process
description: 'Enforces Red-Green-Refactor with hard gates: Red phase writes failing
  test that MUST fail for missing behavior (not syntax/config) and presents test+failure
  before proceeding, runs tests on the specific file (not full suite) after each phase,
  Green phase writes minimal code to pass and stops there, Refactor phase runs test
  after each micro-change and MUST stay Green throughout. Generates failing test cases,
  validates test failure before proceeding, gates implementation behind passing red-phase
  checks. Use when the user requests TDD workflow, asks to write tests first, mentions
