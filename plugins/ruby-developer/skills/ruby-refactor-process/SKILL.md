---
name: ruby-refactor-process
description: 'Enforces a disciplined refactoring process for Ruby code: ONE atomic
  transformation at a time, characterization tests MUST exist and be Green BEFORE
  any edit, run tests after EVERY single step, rollback immediately on Red (no debugging
  in broken state), no behavior changes mixed with refactoring. Covers extract methods,
  rename symbols, split classes, inline variables, remove duplication. Use when: refactor,
  clean up code, rewrite class, structure changes, simplify.'
license: MIT
metadata:
...
| Context | Next Skill |
|---------|----------|
| Establishing baseline coverage | [test-planning-process](../ruby-test-planning-process/SKILL.md) → [tdd-process](../ruby-tdd-process/SKILL.md) |
| Documenting newly extracted APIs | [write-yard-docs](../ruby-write-yard-docs/SKILL.md) |
| Post-refactoring review | [review-process](../ruby-review-process/SKILL.md) |
