---
name: ruby-model-domain
description: "Use when modeling DDD concepts in Ruby: start from domain invariants\
  \ and ownership before choosing patterns (document each concept with its invariant\
  \ example, e.g., cancelled\u2192active state transition guard, and patterns to avoid),\
  \ prefer default Ruby classes over extra abstractions, entity when identity matters,\
  \ value object when equality by value is correct, aggregate root guards state transitions\
  \ as single entry point, domain service for behavior spanning multiple entities,\
  \ application service orchestrates one use case \u2014 hand off to test-planning-process\
  \ and tdd-process before implementation, and only add repository when real persistence\
  \ boundary exists. Covers mapping of entities, aggregates, value objects, domain\
  \ services, application services, repositories, and domain events without over-engineering."
license: MIT
metadata:
...
- [../../resources/ruby/skills/model-domain/assets/examples.md](../../resources/ruby/skills/model-domain/assets/examples.md) — Worked examples for multi-aggregate domains using pure Ruby.
- [../../resources/ruby/skills/model-domain/assets/modeling_template.md](../../resources/ruby/skills/model-domain/assets/modeling_template.md) — Blank output template for a single domain concept.
